...
Medicare seamlessly bridges the gap between structured health schedules and a premium, accessible user experience. Built completely native with Kotlin and powered by an ultra-fast Supabase REST layer, Medicare delivers local notifications, real-time sync, and granular tracking. 
...
Medicare is designed for both individuals and caregivers who need a reliable, modern solution for health management. Key use cases include:
...
<p align="center">
  <img src="screenshots/medicare_calming_teal.png" width="300" alt="Medication Dashboard generated by Stitch MCP">
</p>
...
Medicare operates via a clean model-view decoupled architecture, communicating exclusively over secure HTTPS endpoints natively formatted to Retrofit.
...
Medicare enforces ultra-strict Row-Level Security (RLS) within Supabase. Data requests bypass SDK boundaries through fully native Retrofit models. For self-hosting or deployment, execute the following within the Supabase SQL Editor:
...
2. **Environment Variables:**
   For security, API keys are completely stripped from source control. In the root directory of your app (`d:/projects/Medicare`), create a file titled `local.properties`. Append your Supabase configuration parameters:
   ```properties
